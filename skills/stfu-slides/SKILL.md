...
| `tools/html2gslides/` | Compilateur HTML → Google Slides (voir ci-dessous) |

---

## HTML-first workflow (html2gslides)

Pipeline alternatif : designer les slides en HTML/CSS (itération rapide, WYSIWYG navigateur), puis compiler pixel-perfect vers Google Slides.
...
tools/html2gslides/
    ...
    cli.py                  ← python3 -m html2gslides <deck-dir|slide.html>
    constants.py            ← EMU_PER_PX=9525, canvas dims
    extract.py              ← Playwright: render + DOM walk + bounding rects
    rich_text.py            ← Inline walker: <strong>, <em>, <code> → segments
    color_map.py            ← CSS rgb() → API {red, green, blue} 0-1
    convert.py              ← Segments → createShape + insertText + updateTextStyle
```
...
# Compiler un slide unique
PYTHONPATH=".claude/skills/stfu-slides/tools" python3 -m html2gslides slide.html --slide-id sl001

# Compiler un deck complet
PYTHONPATH=".claude/skills/stfu-slides/tools" python3 -m html2gslides deck/ --output requests.json

# Dry run (résumé sans JSON)
PYTHONPATH=".claude/skills/stfu-slides/tools" python3 -m html2gslides deck/ --dry-run
```

Output : JSON `{"requests": [...]}` prêt pour `batch_update_presentation`.

### Deck HTML structure

```
deck/
    deck.json               ← manifest {name, slides[]}
    theme/brand.css          ← CSS variables STFU
    theme/layouts/_base.css  ← footer
    theme/layouts/*.css      ← layouts (bullets, two-col, table, etc.)
    slides/NN-slug.html      ← slides individuelles
```

Chaque slide HTML est un fichier standalone avec un `<div class="slide">` sur un canvas 960x540px. Le compilateur utilise Playwright pour le rendu et extrait les positions/styles via `getBoundingClientRect()` + `getComputedStyle()`.

### Mapping HTML → Google Slides

| HTML | Type extrait | Google Slides |
|------|-------------|---------------|
| h1, h2, h3, p | text (segments inline) | createShape TEXT_BOX + updateTextStyle FIXED_RANGE |
| ul/ol | list | TEXT_BOX + createParagraphBullets |
| table | table (cellules + styles) | createTable + insertText/updateTextStyle par cellule |
| img | image (src + rect) | createImage |
| div avec background | shape | createShape RECTANGLE + solidFill |
| .slide background | page_bg | updatePageProperties |

Conversion exacte : **1px = 9,525 EMU**.

### Limitations

| Limitation | Fallback |
|-----------|----------|
| Pseudo-elements (::before/::after) | Skip + warning |
| Gradients CSS | Couleur dominante (solid fill) |
| Images locales | Warning — uploader sur Drive manuellement |
| Border-radius | Non supporté par l'API — ignoré |

### Deck de test

Le deck de test (6 slides : cover, content blocks, two-col, table, section divider) était dans l'ancien workspace local (`_workspace/templates/proposals/html-deck/`). Il doit être recréé ou migré vers le Drive si besoin.
