...
- **Dossier lead** (`_workspace/sales/leads/`) : lire le README.md, pré-remplir client, contacts, contexte
- **Dossier projet** (`_workspace/projects/`) : lire le manifest ou README, extraire client et historique
...
| **Oracle** (si nouveau client ou secteur inconnu) | Gemini via `/Users/jesseparot/.local/bin/gemini` | Recherche company, enjeux sectoriels, actualités récentes |
...
- **Two-pass** (recommandé pour les grosses propales) : valider l'outline de l'approche (étape 6) avant le draft complet
- **One-shot** : passer directement au draft complet
...
### Étape 6 — Designer l'approche (phases)

Construire 3-5 phases avec pour chaque :

| Élément | Contenu |
|---------|---------|
| Nom de la phase | Verbe d'action + objet (ex. "Cartographier les parcours") |
| Durée | En semaines ou jours |
| Activités clés | 3-5 activités concrètes |
| Livrables | Ce qui est remis au client en fin de phase |
| Gate de passage | Condition pour passer à la phase suivante |
| Rôles | Qui fait quoi (STFU, client, partenaires) |

**Informer par les missions passées** : reprendre les approches qui ont fonctionné sur des missions similaires (étape 3).

**Si mode two-pass** : présenter l'outline des phases pour validation avant de rédiger le draft complet.
...
1. Dossier lead courant (`_workspace/sales/leads/{lead}/`)
2. Dossier projet courant (`_workspace/projects/{projet}/`)
3. `_workspace/_inbox/` si aucun contexte
...
| Gemini Oracle | Recherche secteur/client | `/Users/jesseparot/.local/bin/gemini -m gemini-2.5-flash -p "..." 2>&1` (timeout 120s) |
...
| Brief très léger | Poser plus de questions à l'étape 2, recommander two-pass |
| Propale urgente | Mode one-shot, sections réduites, signaler les zones à compléter |
...
- Ancien template : `_workspace/templates/proposals/proposition-commerciale.md`
- Exemple réel : `_workspace/projects/totalenergies-arena/brief-propale-arena-sismage.md`

## Exemples d'invocation

**Exemple 1 : depuis un dossier lead**
```
User: (dans _workspace/sales/leads/acme-dupont-innovation/)
User: /propale
...
Claude: "Propale créée : propale-chatbot-interne_draft.md (dans _workspace/_inbox/)
        ..."
```
